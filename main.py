...
# ---------------------------------------------------
    ...
    reader.set_interpretation_overall() # 카드를 뽑은 후 해석을 요약 
    interpretation_overall = reader.interpretation_overall # TarotAI
    clear_screen() # 화면 초기화 
    print("================================================================") # 테두리 
    if lang == 1: # 언어가 영어라면 
        slow_type(
...
""",
    ...
    # pdf generation
    pdf = PDF(f"{name}") # name = username
    pdf.add_page() # 문서에 새로운 페이지를 추가 
    pdf.add_concern(concern) # 고민 추가 
    pdf.add_block(
        first_tarot_card_ascii,
        first_tarot_card[0],
        interpretation_concern_first,
    ) # 타로카드 아스키코드 추가 
    if card_num == 3: # 3개를 뽑는다면 
        pdf.add_block(
    ...
    pdf.add_result(interpretation_overall) # 해석 요약 추가 
    pdf.output(f"{name}_TarotAI_Result.pdf") # .pdf 형식으로 출력 
...
""",
    ...
    while True: # pdf 만든 후, 작별인사 한 후, email 주소 물음 
        try:
            if lang == 1: # 영어라면 
            ...
            if email_confirm == "Y" or email_confirm == "y": # input으로 Y 혹은 y를 입력했을 때 
                break # 끝내기 
        except: # 다른 input을 입력 
            pass 

    # ------------------------------- email 보내기
    receiver_email = email # 사용자 이메일 
    result_pdf = f"{name}_TarotAI_Result.pdf" # pdf 결과 
    if lang == 1: # 영어일 때 
    ...
    while True:
        try:
            email_sender.send_email(receiver_email, subject, body, result_pdf) # 사용자 이메일, 제목, 내용, 해석 결과 전송 
            break
        except:
            if lang == 1:  
                receiver_email = input("Email address is wrong. Please rewrite your email: ")
            else:
                receiver_email = input("이메일 주소가 잘못되었어. 다시 입력해 줄래?: ") # input으로 새 이메일 주소를 입력받고 try로 넘어감  
...
""",
    ...
    if lang == 1: # 영어라면 
        next5 = input_exit("Press Enter to end.  Thank you.") # 감사인사 
    else: # 한국어라면 
